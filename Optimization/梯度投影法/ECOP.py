...
class ecop:
    ...
    def point_init(self, e):
        ...
        elif (E.shape[0] == E.shape[1]) & (np.abs(np.linalg.det(E)) >= e):
            inv = np.linalg.inv(E)
            x = np.dot(inv, E_b)
        ...
        for i in range(A.shape[0]):
            ...
            while cond < 0:
                ...
                if lamb_min <= lamb_max:
                    ...
                    x = x + lamb_max * d
                    cond = (np.dot(current_cond, x) - current_b).min()
                    new_equl = past_group[index][[min_index],:]
                    if E is None:
                        M = new_equl
                    else:
                        M = np.vstack(E, new_equl)
                        if M.shape[0] >= M.shape[1]:
                            return '非行满秩矩阵'
                    P = self.shadow_matrix(M)
                    d = np.dot(P, np.transpose(current_cond, [1, 0]))
                    if np.dot(np.transpose(d, [1, 0]), d).sum() < e:
                        return '条件错误：条件冲突'
